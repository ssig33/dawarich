...
export default class extends BaseController {
  ...
  connect() {
    ...
    this.speedColoredPolylines =
      this.userSettings.speed_colored_routes || false;
    this.center = this.markers[this.markers.length - 1] || [
      52.514568, 13.350111,
    ];
    ...
    this.layerControl = L.control
      .layers(this.baseMaps(), controlsLayer)
      .addTo(this.map);

    // Display photos layer by default
    this.photoMarkers.addTo(this.map);

    // Load photos if integration is configured
    if (
    ...
    this.map.on("overlayadd", async (e) => {
      ...
      if (e.name === "Photos") {
        if (
          (!this.userSettings.immich_url ||
            !this.userSettings.immich_api_key) &&
  ...
  updateFog(markers, clearFogRadius) {
    ...
    requestAnimationFrame(() =>
      drawFogCanvas(this.map, markers, clearFogRadius)
    );
  }
    ...
    this.settingsPanel.onAdd = () => {
      ...
      div.innerHTML = `
        <form id="settings-form" class="w-48 h-144 overflow-y-auto">
          ...
          <div class="join">
            <input type="number" class="input input-ghost join-item focus:input-ghost input-xs input-bordered w-full max-w-xs" id="route-opacity" name="route_opacity" min="0" max="1" step="0.1" value="${
              this.routeOpacity
            }">
          ...
          <div class="join">
            <input type="number" class="join-item input input-ghost focus:input-ghost input-xs input-bordered w-full max-w-xs" id="fog_of_war_meters" name="fog_of_war_meters" min="5" max="100" step="1" value="${
              this.clearFogRadius
            }">
          ...
          <div class="join">
            <input type="number" class="join-item input input-ghost focus:input-ghost input-xs input-bordered w-full max-w-xs" id="meters_between_routes" name="meters_between_routes" step="1" value="${
              this.userSettings.meters_between_routes
            }">
          ...
          <div class="join">
            <input type="number" class="join-item input input-ghost focus:input-ghost input-xs input-bordered w-full max-w-xs" id="minutes_between_routes" name="minutes_between_routes" step="1" value="${
              this.userSettings.minutes_between_routes
            }">
          ...
          <div class="join">
            <input type="number" class="join-item input input-ghost focus:input-ghost input-xs input-bordered w-full max-w-xs" id="time_threshold_minutes" name="time_threshold_minutes" step="1" value="${
              this.userSettings.time_threshold_minutes
            }">
          ...
          <div class="join">
            <input type="number" class="join-item input input-ghost focus:input-ghost input-xs input-bordered w-full max-w-xs" id="merge_threshold_minutes" name="merge_threshold_minutes" step="1" value="${
              this.userSettings.merge_threshold_minutes
            }">
          ...
          <label for="raw">
            <input type="radio" id="raw" name="points_rendering_mode" class='w-4' style="width: 20px;" value="raw" ${this.pointsRenderingModeChecked(
              "raw"
            )} />
            Raw
          </label>
          ...
          <label for="simplified">
            <input type="radio" id="simplified" name="points_rendering_mode" class='w-4' style="width: 20px;" value="simplified" ${this.pointsRenderingModeChecked(
              "simplified"
            )}/>
            Simplified
          </label>
  ...
  createPhotoMarker(photo) {
   ...
   onmouseout="this.firstElementChild.style.boxShadow = '';">
        ...
        <p>Location: ${photo.exifInfo.city}, ${photo.exifInfo.state}, ${
      photo.exifInfo.country
    }</p>
      ...
      const updateMonthLinks = (selectedYear, availableMonths) => {
        ...
        allMonths.forEach((month, index) => {
          ...
          if (availableMonths.includes(month)) {
            ...
            const href = `map?end_at=${encodeURIComponent(
              endDate
            )}&start_at=${encodeURIComponent(startDate)}`;
  ...
  getWholeYearLink() {
    ...
    return `map?end_at=${encodeURIComponent(
      endDate
    )}&start_at=${encodeURIComponent(startDate)}`;
  }
